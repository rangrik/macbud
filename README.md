...
- **Clock:** a 24-hour clock sits on the left of the notch tab, visible while the notch is closed, and beside Keep Alive once it is open. Turn it off in Settings → Features. With the clock on, the MacBud logo moves to the right of the notch.
- **Predicted opening section:** ⌥Space opens on what you most likely want: a screenshot or dictation from the last minute, else Clipboard. With Codex installed, a small model predicts from metadata only and a bigger one learns from misses. Settings → Prediction shows hit rates, usage, the strategies and every prompt and reply; turn Codex off there. Files live in `~/Library/Application Support/MacBud/predict/` ([spec](docs/superpowers/specs/2026-09-24-predictive-opening-section.md)).
