...
enum Automation {
    ...
    private static func handlePrivileged(_ command: String, params: [String: String], app: AppDelegate) async {
        ...
        case "type":
            for ch in params["text"] ?? "" {
                let s = String(ch)
                if let event = makeKeyEvent(characters: s, keyCode: 0, flags: [], window: app.notch.panel) { app.notch.panel.sendEvent(event) }
    ...
    static func keyEvent(for token: String, window: NSWindow) -> NSEvent? {
        ...
        let digitCodes: [String: UInt16] = ["1": 18, "2": 19, "3": 20, "4": 21, "5": 23, "6": 22, "7": 26, "8": 28, "9": 25, "0": 29]
        return makeKeyEvent(characters: keyName, keyCode: digitCodes[keyName] ?? 0, flags: flags, window: window)
    }
