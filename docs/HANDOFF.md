...
Gotchas: a key shelf already followed the pointer because MacBud is frontmost and its focused-window lookup returns nil. `CGRect.contains` excludes the top edge, which QA caught selecting the screen above or falling back to the focused window; the existing chooser test now guards that case.

Verification: independent QA round 2 reported **9 pass / 0 fail**. **136 tests in 25 suites passed** in the unfiltered xcodebuild run, with no `Failing tests:` line. Debug and Release builds passed; Release reported zero warnings and strict signature verification passed. Live on two displays: with focus and pointer on opposite screens, `mbctl toggle` opened the shelf on the pointer’s screen in both directions, including both top rows. Dictation opened on and retargeted to the pointer’s screen; tab hover opened a non-key shelf, leaving closed it, and real tab clicks opened a key shelf on the clicked screen. Retarget traces, app dumps and screenshots matched. Physical hotkey delivery was not exercised; `mbctl` drove the same toggle handler.

Isolation: tests and live QA used disposable homes. Pointer positions were restored, temporary target windows and Debug apps stopped, and all test dictation recordings cancelled without delivery. No owner memory files were edited or prediction sessions/strategies left behind. Earlier concurrent automation interference and existing test-only warnings are recorded in the implementation handoff.
