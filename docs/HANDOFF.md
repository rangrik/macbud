...
Verification: **136 tests in 25 suites passed** in an unfiltered xcodebuild run; Release built without warnings and strict signature verification passed. No new tests: existing KeepAwake tests cover the controller. In the running Debug app, `mbctl` clicks toggled on then off on the shelf; a hover-opened non-key shelf also toggled on one click while remaining non-key. Down opened the island; `keep-awake?enabled=1` reported active on both. Rendered snapshots showed the trailing label without the badge, the mark in color/white, and the unchanged closed band. Launch-only `disabledFeatures` hid the strip on both surfaces; removing that override restored it.

Limits: native UI automation stalled, and ImageRenderer substitutes a placeholder for the native switch. Real ⌘Space, the switch’s on-screen appearance/hover tooltip, and clicking Settings → Features off/on still need the separate QA pass. An initial follow-up click ran after the shelf had closed; an uninterrupted rerun passed. The test target emitted an existing unused `coordinator` warning and Xcode’s AppIntents metadata warning; neither appears in the Release build.

QA used `CFFIXED_USER_HOME=/tmp/macbud-keep-alive-home`; prediction startup was suppressed for the final interaction checks. The native UI tool also launched a second regular instance, which was stopped. No live-data files were edited or cleaned, no clipboard items were copied during UI checks, and any isolated QA sessions remain only in the temporary home. The Debug app is stopped.
