...
Gotcha: a key shelf already followed the pointer because MacBud is frontmost and its focused-window lookup returns nil.

Verification: Debug build passed; **136 tests in 25 suites passed** in the unfiltered xcodebuild run, with no `Failing tests:` line. The existing priority test now covers pointer wins on either display, offscreen-pointer fallback and largest window overlap, and nil when neither input matches. Release built with no reported warnings and strict signature verification passed.

Live verification: `make run` + `mbctl` on built-in display 1 and external display 2. With iPhone Mirroring focused on the external (AX position -687, -1016) and the pointer at (200, 200) on the built-in, `toggle` opened the shelf on display 1 and logged `retarget display=1`. Plain opens followed the pointer on both screens; synthetic tab hover opened a non-key shelf on each, and tab clicks opened on their own screen. Dictation moved an open shelf from display 1 to display 2 and also opened from closed on display 2; both recordings were cancelled. The rendered shelf snapshot was inspected. Physical hotkey delivery remains unverified: the injected Option-Space did not trigger, so the same toggle handler was exercised with `mbctl`. Concurrent automation from another workspace caused focus changes and one tab-click shelf to close; the external click passed on retry.

Isolation: tests used `TEST_RUNNER_CFFIXED_USER_HOME=/tmp/macbud-pointer-qa-home`; live launch used `CFFIXED_USER_HOME` at that same path. The existing Debug app was stopped before pasteboard-writing tests. Temporary app data and the automation log landed under that home; no prediction sessions or strategies were created, and the disposable target window was stopped. No owner memory files were edited. Existing test-only unused-variable and AppIntents metadata warnings are recorded in the implementation handoff, outside this change.

---

# Latest update — Jev in the shadow of the driver (2026-09-25, 0.8.4)

An experiment: can TypeSafe's Jev (`jev-latest`, typed choices with probabilities in ~150 ms, no text) replace gpt-6-luna as the driver ([record](superpowers/specs/2026-09-25-jev-shadow-driver.md))? Offline, `scripts/jev-replay.py` replayed the 10 scored opens and 6 probes; too few to decide, and the reviewer's rules were written from those same opens. So Jev now runs in the shadow: `JevRunner` (a `ModelRunner` whose "prompt" is the request body from `PredictionPrompts.jev`, with ages as words because Jev reads numbers as text) is asked on the same context changes as the driver, with the same rules and history; `SectionPredictor.shadowRefresh` caches its pick, `intentForOpen` copies it onto the open as `SessionRecord.shadow`, `sessionEnded` scores it as `shadowHit`. It never lands. `rates` gains `shadow` and `landedWhereShadow`; Settings → Prediction shows both when the key file exists; each open's Activity details get a "Jev, in the shadow" line. Shadow calls are `purpose: "shadow"` in `calls.jsonl`, outside the daily cap, and a failed one does not flip the driver's status. The key is `~/Library/Application Support/MacBud/typesafe-api-key` (owner-only); without it there are no calls.

Gotchas: `grep -v 'a\|b' file > out` under the rtk hook wrote 8 lines of a 90-line `predictions.jsonl`; it was restored from a backup taken first. Edit memory files with Python. The driver was at its daily cap of 100 during QA, so the landing came from the rules.

Verification: **136 tests in 25 suites passed** (new seams: the Jev request carries metadata only; Jev's answers → the driver's reply shape → a scored shadow that never lands, keeps the status and stays outside the cap). Live QA on the Debug app with the real key: shadow calls at 245, 143 and 126 ms with 1,568 to 1,626 input tokens, one per context key; an open through `mbctl` with ↩ on the top card recorded `shadow: text/any 0.95`, `shadowHit: true` next to the rules' hit. The two QA opens were removed from `predictions.jsonl`; the call ledger was kept. The Settings row was not inspected on screen.
